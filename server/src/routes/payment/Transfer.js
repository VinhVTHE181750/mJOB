...
const transfer = async (req, res) => {
  ...
  try {
    ...
    if (!balanceFrom) {
      ...
      await logPayment(req.userId, "TRANSFER", amount, req.userId, userTo.id, "FAILED");
      return res.status(400).json({ message: "Insufficient balance", balance: 0 });
    ...
    await logPayment(req.userId, "TRANSFER", amount, req.userId, userTo.id, "SUCCESS");
    await logPayment(userTo.id, "TRANSFER", amount, req.userId, userTo.id, "SUCCESS");

    getIo().emit(`payment/${req.userId}`, { balance: balanceFrom.balance });
    return res.json({
